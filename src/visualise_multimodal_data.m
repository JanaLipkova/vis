...
for i = 1:length(vis.Modalities)
    ...
    % get data
    inputPath = [vis.InputFolder, vis.Modalities{i}];
    ...
    modName   = baseName{1}(1:end-vis.NameCutBy);
    
    % plot on given position
    axes( hAxes{vis.nRows - row + 1,col} )
    pcolor(data.vol(:,:,vis.Slide));
    daspect([1 1 1])
    shading flat;
    ...
    % title
    v = axis;
    posX = v(1) + 0.45* (v(2) - v(1));
    posY = v(3) + 0.95* (v(4) - v(3));
    text( posX, posY, modName, 'FontSize', 20, 'FontWeight','bold','Color', [0.99,0.99,0.99],'Interpreter','none' );
...
print(gcf,[vis.OutputFolder, vis.OutputName],'-djpeg')
